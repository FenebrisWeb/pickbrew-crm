...
function show_crm_dashboard() {
    ...
    ob_start(); 
    ?>
    <div style="font-family:'Inter', sans-serif; max-width:1400px; margin:40px auto; background:#fff; padding:30px; border-radius:12px; box-shadow:0 4px 20px rgba(0,0,0,0.05); overflow-x: auto;">
...
function show_crm_form() {
    ...
    if ( isset($_POST['submit_crm_entry']) && is_user_logged_in() ) {
        ...
        if(empty($errors)) {
            ...
            // B. Save Post Title (Business Name)
            ...
            if ( isset($_POST['send_mockup_check']) && $_POST['send_mockup_check']=='yes' ) {
                ...
                $th = sanitize_text_field($_POST['mockup_theme']);
                $u = sanitize_text_field($_POST['client_username']);
                $pw = sanitize_text_field($_POST['client_password']);
                wp_mail( $to, "Your $th Theme Mockup", "Hello,\n\nHere are your login details:\nUser: $u\nPass: $pw" );
            }
            
            echo '<script>window.location.href="/crm/";</script>'; exit;
        } else {
    ...
    <style>
        ...
        .crm-input, .crm-select, .crm-textarea { width:100%; padding:12px; border:1px solid #ccc; border-radius:4px; font-size:14px; box-sizing:border-box; background: #fff; }
        .crm-input, .crm-select { height: 45px; }
    ...
    <form method="post" id="crmForm" novalidate>
        ...
        <div id="wrap_date_signed" class="conditional-box">
            ...
            <input type="date" name="date_signed" class="crm-input logic-req" value="<?php echo gv('date_signed',$db); ?>">
        </div>
        ...
        <div class="crm-full">
            ...
            <div id="box_flat" style="display:none; margin-top:10px;">
                ...
                <select name="monthly_fee_val" class="crm-select">
                    ...
                    <?php 
                    // FIXED LOGIC: Store clean number, display with $. Robust check against DB value.
                    $fees = ['299.99', '349.99', '399.99', '449.99', '499.99', '599.99']; 
                    foreach($fees as $f) { 
                        // We strip $ from DB value just in case to compare number-to-number
                        $db_clean = str_replace('$', '', gv('monthly_fee_val',$db));
                        $sel = ($db_clean == $f) ? 'selected' : ''; 
                        // Value stored is clean number: "299.99"
